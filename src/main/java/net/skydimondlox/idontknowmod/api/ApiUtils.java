...
import net.skydimondlox.idontknowmod.api.utils.TagUtils;
...
import static net.skydimondlox.idontknowmod.api.IDKTags.getIngot;
...
public class ApiUtils
{
    ...
    {
        String[] keys = API.prefixToIngotMap.keySet().toArray(new String[0]);
        String[] type = TagUtils.getMatchingPrefixAndRemaining(tags, stack, keys);
        if(type!=null)
        {
            Integer[] relation = API.prefixToIngotMap.get(type[0]);
            if(relation!=null&&relation.length > 1)
            {
                double val = relation[0]/(double)relation[1];
                return Pair.of(API.getPreferredTagStack(tags, TagUtils.createItemWrapper(getIngot(type[1]))), val);
            }
        }
        return null;
    }
